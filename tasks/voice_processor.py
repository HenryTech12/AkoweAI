...
def process_voice(user_id: str, audio_s3_key: str, dialect: str):
    """
    ...
    try:
        ...
        # Download audio from Cloudinary (run in event loop)
        audio_bytes = asyncio.run(download_file(audio_s3_key))
        logger.info(f"Downloaded audio from Cloudinary: {audio_s3_key}")

        # Transcribe audio
        transcription = asyncio.run(transcribe_audio_from_bytes(
            audio_bytes,
            audio_s3_key.split('/')[-1],
            language=_get_whisper_language_code(dialect)
        ))
        logger.info(f"Transcribed audio: {transcription[:100] if transcription else 'N/A'}...")

        # Detect dialect if not provided
        detected_dialect = asyncio.run(analyze_dialect(transcription)) if dialect == "auto" else dialect
